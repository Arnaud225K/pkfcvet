...
def tracking_id(request):
    try:
        ...
        tracking_id_bytes = base64.b64encode(os.urandom(36))
        tracking_id_str = tracking_id_bytes.decode('ascii')
        request.session['tracking_id'] = tracking_id_str
        return request.session['tracking_id']
...
def recommended_from_search(request):
    ...
    from search import searchpy as search
    matching = []
